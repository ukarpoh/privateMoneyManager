...
async def handle_expense(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Intercept: waiting for custom date input ──────────────────────────────
    ...
    # ── Intercept: waiting for an edit field value ────────────────────────────
    ...
    if edit_state:
        ...
        if field == "amount":
            ...
            if parsed_amount is None or parsed_amount <= 0:
                await update.message.reply_text(
                    "Invalid amount. Enter a positive number (e.g. `15.50`).",
                    parse_mode="Markdown",
                )
                return
            value = parsed_amount
        ...
        refreshed_row = db.get_expense(expense_id)
        if prompt_msg_id and prompt_chat_id and refreshed_row:
            card_text, card_kb = _build_edit_card(refreshed_row, currency)
            try:
                await context.bot.edit_message_text(
                    chat_id=prompt_chat_id,
                    message_id=prompt_msg_id,
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ AI service authentication failed. Check `KIMI_API_KEY` in your `.env` file."
        )
        return
